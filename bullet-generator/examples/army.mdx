...
Built for the NCOER under the six-attribute model: **Character**, **Presence**, **Intellect**, **Leads**, **Develops**, and **Achieves**. Pick one attribute and the three options are shaped to demonstrate that behavior with action / result / impact. Outputs use NCOER bullet style (no personal pronouns, abbreviated where natural) and stay within typical NCOER comment-block character targets, with `xx` placeholders for any unsupplied numbers.
...
Built for **DA Form 67-10-1**, company-grade officers (2LT-CPT). Section-aware — pick one of nine OER surfaces and the three options are shaped to that section's purpose:
...
Output is written in OER narrative style (full sentences, not bullets), grounded in the rater's perspective, and uses `xx` placeholders when specific numbers aren't supplied. Promotion-recommendation language is never fabricated.
...
**Example Output (Option 1):**
LT [name] led with calm, decisive judgment during a demanding NTC rotation; coordinated xx logistics packages across xx supported platoons, built training plans for xx new lieutenants, and reinforced standards that produced measurable improvements in company-level execution and readiness.

## OER - Tailored (Field Grade)

Built for **DA Form 67-10-2**, field-grade officers (MAJ-LTC). Section-aware — extends the company-grade attribute set with field-grade-specific surfaces:
...
Output emphasizes battalion- and brigade-level responsibility, joint / multinational impact, and trajectory toward command and key billet selection.
...
Built for **DA Form 67-10-3**, strategic-grade and Colonel-level officers (COL). Section-aware with a strategic emphasis:
...
Output emphasizes strategic context, combined / coalition / interagency complexity, and readiness for general officer / strategic leader assignments.
...
**Example Output (Option 1):**
COL [name] delivered exceptional brigade command through a complex CTC rotation, integrating joint and multinational forces across xx warfighting functions and building a leader development program that prepared xx field-grade officers for follow-on command. Strategic judgment, presence, and proven ability to lead at echelon support continued growth toward senior strategic responsibility consistent with the senior rater's assessment.
